...
3. **Read `./growth-engine/ops-workflow.md`.** It names the snapshot they chose and holds the copy already written for it. Reuse that copy rather than writing a second version of the same email.
   - If it does not exist, build the operations engine first (`/growth-engine:ops`, or "find my bottleneck"). The snapshot choice lives there.
...
- **Nothing in curly or square brackets, anywhere in a value.** No `[first name]`, no merge field code. If the copy being reused carries a placeholder like `[first name]`, take it out here: the step supplies the greeting and the name, and a value holds only the words that are the same for everyone.
