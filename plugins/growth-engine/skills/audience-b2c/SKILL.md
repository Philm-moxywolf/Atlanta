...
Write the copy for each. The workflow itself is a GoHighLevel snapshot loaded at the clinic, so do not attempt to build automation here. Copy only. These words become the snapshot's custom values, filled in after it loads, so write no greeting into a message and nothing in square or curly brackets.
