...
| 4 | **The brain** | `bin/ge`: schema'd state, one writer per file, snapshot before every overwrite with undo, an append-only ops log, a derived index, and a doctor that prints evidence rather than assertions |
